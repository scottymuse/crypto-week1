#!/usr/bin/python

import sys

cts = [
	,
	0x234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f,
	0x32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb,
	,
	0x3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070,
	,
	,
	,
	,
	0x466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83,
	0x32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904
				]

msgs = []

asciis = [0x20] + [0x3a] + range(0x41,0x5b) + range(0x61,0x7b)

def strxor(a, b):     # xor two strings of different lengths
    if len(a) > len(b):
        return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a[:len(b)], b)])
    else:
        return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a, b[:len(a)])])

def ctxor(a, b):
	if a.bit_length() > b.bit_length():
		return a ^ (b<<(a.bit_length() - b.bit_length()))
	else:
		return (a<<(b.bit_length() - a.bit_length())) ^ b

def testkey(i, key):
#	if i == 0:
#		print "Trying:", hex(key)
	fails = 0
	for ct in cts:
		try:
			byte = (ct >> (ct.bit_length() + (8 - ct.bit_length()%8) - ((i + 1) * 8))) & 0xff
		except ValueError:
			continue
		if byte ^ key not in asciis:
			fails += 1
#			if i == 0:
#				print "Failed on:", hex(byte)
#	print hex(byte ^ key)
	return fails

def getkey():
	key = 0
	for i in range(max(cts).bit_length()/8):
		foundkey = False
		print "*****************", i, "********************"
		for j,ct1 in enumerate(cts):
			for k,ct2 in enumerate(cts[j+1:]):
#				print hex(ct1)
#				print hex(ct2)
				try:
					byte1 = (ct1 >> (ct1.bit_length() + (8 - ct1.bit_length()%8) - ((i + 1) * 8))) & 0xff
					byte2 = (ct2 >> (ct2.bit_length() + (8 - ct2.bit_length()%8) - ((i + 1) * 8))) & 0xff
				except ValueError:
					continue
				val = byte1 ^ byte2
				print hex(byte1), hex(byte2), hex(val)
				fails = {}
				for guess in asciis:
#					print hex(guess ^ val), (guess ^ val) ^ byte2, guess ^ byte1
					if (guess ^ val in asciis):
						fails[guess] =  testkey(i, guess ^ byte1)
				minkey = min(fails, key=fails.get)
				if i == 0:
					print fails
					print minkey
				if fails[minkey] < 2:
#				print hex(guess ^ byte1)
					if i == 0:
						key = minkey ^ byte1
					else:
						key <<= 8
						key += minkey ^ byte1
					foundkey = True
				if foundkey:
					break
			if foundkey:
				break
		if not foundkey:
			print i, "no key found"
			key <<= 8
		print hex(key)
	return key

key = getkey()
print hex(key), key.bit_length()
for ct in cts:
	print ct.bit_length()

for i, ct in enumerate(cts):
#	msg = None
#	print hex(ct)
#	if ct.bit_length() > key.bit_length():
#		msg = ct ^ (key << (ct.bit_length() - key.bit_length()))
#	else:
#		msg = (ct << (key.bit_length() - ct.bit_length())) ^ key
#	print hex(msg)
	asciimsg = ""
	for byte in range((ct.bit_length() + (8 - ct.bit_length()%8))/8):
		ctbyte = (ct >> (ct.bit_length() + (8 - ct.bit_length()%8)) - byte*8) & 0xff
		keybyte = (key >> (key.bit_length() + (8 - key.bit_length()%8)) - byte*8) & 0xff
		if i == 0:
				print hex(ctbyte), hex(keybyte)
		if keybyte == 0x00:
			asciimsg += "*"
		else:
			asciimsg += chr(ctbyte ^ keybyte)
	print asciimsg
	print
